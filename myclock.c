...
** by Pf. mmurphy
** I modified it and remove name&host output in /var/log/message and proc file, 
** the modueles create a proc file and write a pair of sec and milisec into the proc file
** When a process call the read() syscall, the string of time
** (sec&milisec/1000 separated by a single empty space) will be return 
** I implement a User interface program later to diplay a formatted time 
** (e.g.Feb 28 2011 2:22:755 pm)once per sec in 5 minute period.
**	
*/
...
/* Since this function calls the read function which doesn't        
* have terminated indicator, the modules function has to stop 
...
*/
	if (offset > 0) {
		...
		ret = sprintf(buffer, "%d, %d\n", (int)CURRENT_TIME.tv_sec,(int)CURRENT_TIME.tv_nsec/1000);    //Here I added my name and the hostname
	}
	return (ret);
}
